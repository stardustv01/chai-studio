...
Version `1.0.0-rc.4` has one measured production class: macOS 26.x on arm64 Apple M4 with 16 GB unified memory and integrated M4 GPU, Node 22.17.0, FFmpeg/FFprobe 7.1.1, Playwright-managed Chromium 1228 for UI QA, and the matching managed headless shell for engine work. `fixtures/release/support-matrix.json` is the machine-readable authority.
